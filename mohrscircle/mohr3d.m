...
if(prompt==1)
   ...
   title('State of stress: {sxx, syy, txy} = {100, 0, 0} [Units] ')
else
...
% axis([-2*max(s_1,s_2) 2*max(s_1,s_2) -1.25*max(s_1,s_2) 1.25*max(s_1,s_2)]) %Sets axis limits to improve aesthetics of figure.
pbaspect([1 1.25 1])
hold on;
