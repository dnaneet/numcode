...
%{
John Kitchin (jkitchin@andrew.edu)
...
classdef (InferiorClasses = {?double}) unit < double
    ...
    methods
        function U = unit(data,exponents,displaystring)
            ...
            %
            % some examples:
            % U = unit(1,'m','m') defines a base unit of a meter
            % U = unit(0.5,[1 -1 0 0 0 0],'m/s') defines 0.5*m/s
            
            U = U@double(data); %initialize superclass with the data
                        
            if nargin > 1
                if isempty(unit.base_units)
                    base_units = unit.base_units('SI');
                else
        ...
        function s = display(U,format)
            ...
            if n == 1
                ...
                for i=1:numel(e) %loop over each exponent
                    ...
                    if abs(e(i)) > unit.TOLERANCE
                        %check if it is an integer
                        if abs(mod(e(i),1)) < unit.TOLERANCE
                            % it is an integer. check if it is one
                            if abs(e(i)-1) < unit.TOLERANCE
                                s = strcat(s,'*%s'); %+1
                                s = sprintf(s,unit.base_units{i});
                            elseif abs(e(i)+1) < unit.TOLERANCE
                                s = strcat(s,'/%s'); %-1
                                s = sprintf(s,unit.base_units{i});
                            elseif e(i) > 1
                                s = strcat(s,'*%s^%i');
                                s = sprintf(s,unit.base_units{i},e(i));
                            elseif e(i) < -1
                                s = strcat(s,'/%s^%i');
                                s = sprintf(s,unit.base_units{i},abs(e(i)));
                            end
                        else
                            % exponent is a float
                            s = strcat(s,'*[%s^%g]');
        ...
        function varargout = as(U1, U2, format)
            ...
            for i = 1:m
                for j = 1:n
                    if any(abs(e1{i,j} - e2{1})) > unit.TOLERANCE
        ...
        function U = plus(U1,U2)
            ...
            if isa(U1,'unit') && isa(U2,'unit')
                ...
                if (numel(e1) == 1)
                    for i=1:m
                        for j=1:n
                            if any(abs(e1{1,1} - e2{i,j})) > unit.TOLERANCE
                ...
                elseif (numel(e2) == 1)
                    for i=1:m
                        for j=1:n
                            if any(abs(e1{i,j} - e2{1,1})) > unit.TOLERANCE
                                error('units:plus','incompatible units for addition')
                            end
                        end
                    end
                    ...
                    for i=1:m
                        for j=1:n
                            if any(abs(e1{i,j} - e2{i,j})) > unit.TOLERANCE
                ...
                e = e1;
                ds = U1.displaystring;
            elseif ~isa(U1,'unit') && isa(U2,'unit')
                e = U2.exponents;
                ds = U2.displaystring;
            elseif isa(U1,'unit') && ~isa(U2,'unit')
                e = U1.exponents;
        ...
        function U = mtimes(U1, U2)
            ...
            if ~isa(U1,'unit')
                % constant * U2
                %'case a'
                U = unit(U1*double(U2),U2.exponents,U2.displaystring);
                return
            elseif ~isa(U2,'unit')
                % U1*constant
                %'case b'
                U = unit(double(U1)*U2,U1.exponents,U1.displaystring);
                return
            end
                        ...
                        for i = 1:m1
                            for j = 1:n2
                                ...
                                if isa(Uij,'unit')
                                    ...
                                    U = subsasgn(U,s,unit(Uij,{[0 0 0 0 0 0 0]},''));
                                end
                            end
                        end
        ...
        function U = mrdivide(U1, U2)
            ...
            elseif isa(U1,'unit') && ~isa(U2,'unit')
                % unit/number
                U = unit(v,U1.exponents,U1.displaystring);
            elseif isa(U1,'unit') && isa(U2,'unit')
                % unit/unit
                e1 = U1.exponents;
                e2 = U2.exponents;
        ...
        function U = horzcat(varargin)
            ...
            elseif isempty(u1) && isa(u2,'unit')
                exps = [u2.exponents];
            elseif isempty(u2) && isa(u1,'unit')
                exps = [u1.exponents];
            else
                exps = {[0 0 0 0 0 0 0]};
            ...
            switch s(1).type
                ...
                case '()'
                    ...
                    if all([em en] == [1 1])
                        ...
                        sexps = exps;
                        out = unit(sdata,sexps,U.displaystring);
        ...
        function Z = trapz(X,Y,DIM)
            if nargin == 1
                Y = X;
                Z = unit(trapz(double(Y)),Y.exponents, Y.displaystring);
            elseif nargin == 2
                if isscalar(Y)
                    DIM = Y; Y = X;
                    Zd = trapz(double(Y),DIM);
                    Z = unit(Zd,Y.exponents,Y.displaystring);
                else
                    Zd = trapz(double(X),double(Y));
                    U = X*Y';
                    Z = unit(Zd,U.exponents,U.displaystring);  
                end
            elseif nargin == 3
                % cumtrapz(x,y,dim)
        ...
        function Z = cumtrapz(X,Y,DIM)
            if nargin == 1
                % cumtrapz(Y)
                Y = X;
                Z = unit(cumtrapz(double(Y)),Y.exponents,Y.displaystring);
        ...
        function yiu = interp1(varargin)
            ...
            yiu = unit(yi,Yu.exponents,Yu.displaystring);
        end
        ...
        function U = sum(U1,DIM)
            ...
            U = unit(v,U1.exponents);
   
        end
        
        function U = floor(U1)
            U = unit(floor(double(U1)),U1.exponents,U1.displaystring);   
        end
        
        function U = ceil(U1)
            U = unit(ceil(double(U1)),U1.exponents,U1.displaystring);            
        end
        
        function U = round(U1)
            U = unit(round(double(U1)),U1.exponents,U1.displaystring);   
        end
        
        function U = fix(U1)
            U = unit(fix(double(U1)),U1.exponents,U1.displaystring);   
        end
        ...
        function [varargout] = min(varargin)
            ...
            if nargin == 1
                ...
                if nargout <= 1
                    varargout{1} = unit(Y,U.exponents,U.displaystring);
                elseif nargout == 2
                    varargout{1} = unit(Y,U.exponents,U.displaystring);
                    varargout{2} = I;
                end
            ...
            elseif nargin == 2
                ...
                varargout{1} = unit(Y,U1.exponents,U1.displaystring);
        ...
        function [varargout] = max(varargin)
            ...
            elseif nargin == 2
                ...
                if isa(U1,'unit')
                    ...
                    varargout{1} = unit(Y,U2.exponents,U2.displaystring);
                end
        ...
        function C = eq(U1,U2)
            ...
            if isa(U1,'unit') && isa(U2,'unit')
                ...
                if m1 == m2 && n1 == n2
                    ...
                    for i=1:m1
                        for j=1:n1
                            CE(i,j) = all(abs(e1{i,j} - e2{i,j})< unit.TOLERANCE);
                        end
                    end
                    C = C & CE;
                end
        ...
        function  [P,S,MU] = polyfit(X,Y,N)
            ...
            % units on parameters           
            P = [1 1]*unit(1,{[0 0 0 0 0 0 0]},'');
            for i=1:N+1
                % the ith element is P(i)*X^(N - (i - 1)) and the
                % units on that are the units of Y
                x1 = X(1);
                y1 = Y(1);
                if isa(X,'unit') && isa(Y,'unit')
                    % make unit units to compute units on parameter
                    ux = unit(1,x1.exponents,x1.displaystring);
                    uy = unit(1,y1.exponents,y1.displaystring);
                    U = uy/(ux^(N-(i-1)));
                elseif isa(X,'unit')
                    %'only units on x'
                    ux = unit(1,x1.exponents,x1.displaystring);
        ...
        function pder = polyder(P)
            if nargin > 1
                error('unit','polyder only supports one polynomial with units')
            end
        ...
        function pint = polyint(P,K)
           ...
           error('unit','polyint not supported for units') 
        end
        
        function pval = polyval(P,X)
            if nargout > 1
                error('unit','delta output is not supported yet')
            end
        ...
        function out = reshape(U,varargin)
            ...
            out = unit(data,U.exponents,U.displaystring);
        end
        ...
        function str = sprintf(format,varargin)
            ...
            if numel(match) ~= numel(varargin)
                error('unit','inputs do not match number of formats')
            end
        ...
        function U = transpose(U1)
            ...
            U = unit(data,U1.exponents',U1.displaystring);
        end
        ...
        function varargout = ode45(fh,tspan,init,options)
            ...
            function odeout = odefun(fh,t,y)
                ...
                t1 = tspan(1);
                t = unit(t,t1.exponents);
                y = unit(y,init.exponents);
            ...
            if nargout == 1
                ...
                sol = varargout{1};
                t1 = tspan(1);
                x = unit(sol.x,t1.exponents);
                y = sol.y;
                % this code allows there to be different units.
                for i=1:length(init)
                    u = unit(y(i,:),init.exponents{i});  
                    Y(i,:) = u;
                end
            ...
            elseif (nargout >= 2)
                ...
                y = varargout{2};
                t1 = tspan(1);
                varargout{1} = unit(t,t1.exponents);
            ...
            elseif (nargout == 5)
                ...
                YE = varargout{4};
                t1 = tspan(1);
                varargout{3} = unit(TE,t1.exponents);
                warning('unit','check units on YE. they may not be correct')
                varargout{4} = unit(YE,init.exponents);
            end
        end
        ...
        function varargout = ode15s(fh,tspan,init,options)
            ...
            if nargout == 1
                ...
                x = unit(sol.x,tspan(1).exponents);
                y = sol.y;
                % this code allows there to be different units.
                for i=1:length(init)
                    u = unit(y(i,:),init.exponents{i});  
        ...
        function [uxint,upxint] = deval(sol,xint,idx)
            ...
            if nargin == 2
                ...
                if nargout == 1
                    ...
                    uxint = unit(uxint,y.exponents,y.displaystring);
                else
                    [uxint,upxint] = deval(sold,double(xint));
                    uxint = unit(uxint,y.exponents,y.displaystring);
                    dydx = (sol.y)./(sol.x);
                    upxint = unit(upxint,dydx.exponents,dydx.displaystring);
                end
            elseif nargin == 3
                sold = sol;
                sold.x = double(sol.x);
        ...
        function varargout = meshgrid(varargin)
            ...
            for i = 1:nargin
                varargout{i} = unit(meshout{i},meshunit{i});
            end
            if (nargout > nargin)
                varargout{end} = unit(meshout{end},meshunit{end});
            end
        end
        ...
        function [q,varargout] = quad(fun,a,b,varargin)
            ...
            function quadfunout = quadfun(fh,x)
                ...
                if isa(a,'unit')
                    x = unit(x,a.exponents,a.displaystring);
                end
            ...
            if (nargin >= 1)
                q = unit(quadout{1},funit.exponents,funit.displaystring);
            end
        ...
        function [x,varargout] = fsolve(varargin)
            ...
            function nlaout = nlafun(fh,x)
                % Make sure there are units on x
                x = unit(x,x0.exponents,x0.displaystring);
            ...
            if nargout == 0
                ...
                x = unit(fsolveout{1},x0.exponents,x0.displaystring);
            end
            
            % Process outputs
                    
            if (nargout >= 2)
                varargout{1} = unit(fsolveout{2},fvalunit.exponents,fvalunit.displaystring);
            end
            if (nargout >= 3)
        ...
        function [x,varargout] = fzero(varargin)
            ...
            if nargout == 0
                ...
                x = unit(fzeroout{1},x0.exponents,x0.displaystring);
            end
            if (nargout >= 2)
                varargout{1} = unit(fzeroout{2},fvalunit.exponents,fvalunit.displaystring);
            end
            if (nargout >= 3)
        ...
        function y = linspace(a,b,varargin)
            ...
            % Put units back on output
            y = unit(y,a.exponents,a.displaystring);
        end
        ...
        function boolean = gt(U1,U2)
            ...
            e1 = U1.exponents;
            e2 = U2.exponents;
            if any(abs(e1{:} - e2{:})) > unit.TOLERANCE
                error('units do not match')
            end
    ...
    methods (Static)
        ...
        function u = units(varargin)
            ...
            %
            % typical usage:
            % u = unit.units;      %returns SI units
            % u = unit.units('CGS')  % returns a units with cm, gm, s as
            % base units.
            % u = unit.units('mm','min','ton','R','mol','coul','candela') % sets the
            ...
            if nargin >= 1
                base_units = unit.base_units(varargin{:});
            else
                if isempty(unit.base_units)
                ...
                for j=1:length(all_units)
                    ...
                    if isKey(map,base_units{i})
                        ...
                        for jj = 1:length(map)
                            ...
                            data = v(jj);
                            disp_string = k{jj};
                            u(1).(disp_string) = unit(data,base_units{i},disp_string); 
                        end
                    end
                end
            end
            ...
            u.degC = @unit.degC;
            u.degF = @unit.degF;
            u.degF2C = @unit.degF2C;
            u.degC2F = @unit.degC2F;
            u.degF2R = @unit.degF2R;
            u.degC2R = @unit.degC2R;
            ...
            u.dimensionless = unit(1,[0 0 0 0 0 0 0],'');
        end
        ...
        function c = constants(base)
            ...
            % same as the base units for unit.units.
            %
            % c = unit.constants;
